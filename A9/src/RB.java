...
class RBNode<E> extends RedBlackT<E> {
    ...
    Optional<Integer> isValidBlackTree() {
        //If not valid returns empty optional, if it is, return Optional<height>
        if (this.color.equals(Color.BLACK)) {
            int leftH = this.RBLeft().isValidTree().get();
            int rightH = this.RBRight().isValidTree().get();
            if(leftH == rightH){
                return Optional.of(1 + leftH);
            }
    ...
    Optional<Integer> isValidRedTree() {
        if(this.color.equals(Color.RED)){
            ...
            int rightH = this.RBRight().isValidTree().get();
            if(leftH == rightH){
                return this.RBLeft().isValidBlackTree();
            }
            else{
                return Optional.empty();
            }
